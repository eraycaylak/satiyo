...
export function ListingCard({ listing, width }: { listing: Listing; width: number }) {
  ...
  return (
    ...
    >
      <View style={{ aspectRatio: 4 / 3, backgroundColor: t.surface2 }}>
        {cover ? <Image source={{ uri: cover }} style={{ width: "100%", height: "100%" }} resizeMode="cover" /> :
          <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}><Text style={{ fontSize: 36, opacity: 0.35 }}>🖼️</Text></View>}
        <Pressable onPress={toggleFav} hitSlop={8}
          ...
          <Text style={{ fontSize: 16, color: fav ? t.danger : "#888" }}>{fav ? "♥" : "♡"}</Text>
        </Pressable>
      </View>
