...
export function ListingDetail({ id }: { id: string }) {
  ...
  async function shareTo(net: "wa" | "x" | "fb" | "ig" | "copy") {
    const url = await getShareUrl();
    const txt = `${listing!.title} — ${formatPrice(listing!.price, listing!.priceType)} · Satıyo'da`;
    // SADECE link → alıcı uygulama zengin OG kartını (ad+foto+açıklama) kendi açar; metin gömülmez (tekrar önlenir).
    if (net === "wa") window.open(`https://wa.me/?text=${encodeURIComponent(url)}`, "_blank", "noopener");
      ...
      // Instagram linki hikayeye otomatik koydurmaz → linki kopyala + Instagram'ı aç; kullanıcı yapıştırır.
      try { await navigator.clipboard.writeText(url.trim()); } catch { /* yoksay */ }
      window.open("https://www.instagram.com/", "_blank", "noopener");
      alert("Link kopyalandı — Instagram hikayene yapıştır 👍");
    }
    else {
      ...
      <div className="detail-grid">
        <div style={{ position: "relative" }}>
          ...
          <div className="gal-actions">
            ...
            <button className="gal-act" onClick={() => shareTo("ig")} aria-label="Instagram'da paylaş" title="Instagram">
              <svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="#E4405F" strokeWidth="2"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r="1.2" fill="#E4405F" stroke="none"/></svg>
            </button>
